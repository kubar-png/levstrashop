...
function BrandLockup({ color, accent, shadow, size = 40 }: { color: string; accent: string; shadow: boolean; size?: number }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: Math.round(size * 0.26) }}>
      ...
      <div style={{ display: 'flex', fontFamily: 'Forum', fontSize: size, color, letterSpacing: 0.5, textShadow: shadow ? '0 1px 10px rgba(0,0,0,0.45)' : 'none' }}>
        ciaobag
      </div>
    </div>
  );
...
function PriceLine({ spec, color, shadow, size = 48 }: { spec: VariantSpec; color: string; shadow: boolean; size?: number }) {
  ...
  const ts = shadow ? '0 2px 12px rgba(0,0,0,0.7)' : 'none';
  return (
...
function PhotoScrims() {
  return (
    <>
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 300, display: 'flex', backgroundImage: 'linear-gradient(180deg, rgba(0,0,0,0.46) 0%, rgba(0,0,0,0) 100%)' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: '62%', display: 'flex', backgroundImage: 'linear-gradient(180deg, rgba(0,0,0,0) 0%, rgba(0,0,0,0.46) 42%, rgba(0,0,0,0.88) 100%)' }} />
    </>
  );
}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 6 }}>
            {spec.benefits.slice(0, 3).map((b, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ display: 'flex', fontFamily: 'Poppins', fontWeight: 600, fontSize: 34, color: WHITE, textShadow: '0 1px 8px rgba(0,0,0,0.6)' }}>{b}</div>
              </div>
            ))}
          </div>
        ) : null}
        <div style={{ display: 'flex', fontFamily: 'Poppins', fontWeight: 700, fontSize: tall ? 86 : 72, lineHeight: 1.05, color: WHITE, maxWidth: 920, textShadow: '0 2px 20px rgba(0,0,0,0.8)' }}>
          {spec.headline}
        </div>
